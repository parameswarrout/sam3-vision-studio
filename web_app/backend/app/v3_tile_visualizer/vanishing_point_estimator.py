...
class VanishingPointEstimator:
    """
    RANSAC-Based Architectural Vanishing Point & Perspective Homography Estimator (V3.0).
    ...
    def estimate_vanishing_point(
        ...
        valid_lines: List[Tuple[float, float, float]] = []
        line_segments = []
        ...
        # 5. Score Candidates by Consensus Voting
        ...
        api_logger.info(
            f"[V3 VanishingPoint] Detected VP=({final_vp_x}, {final_vp_y}) from {len(valid_lines)} lines with confidence={confidence:.2f}"
        )
