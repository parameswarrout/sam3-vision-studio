...
# ID Aliases for backwards compatibility
ID_ALIASES = {
...
def generate_tile_texture(tile_id: str, size: int = 512) -> Image.Image:
    """
    Loads the real MyTyles product texture from disk.
    If not found, synthesizes a fallback texture.
    """
    ...
    # Fallback to procedural generation if file not on disk
...
def ensure_all_tile_textures(output_dirs: List[Path]):
    """Ensures all tile textures exist in target output directories."""
