...
import { Sparkles, Sliders, ArrowRight, Upload, Image as ImageIcon } from "lucide-react";
...
export function TextPromptTab({
  ...
  return (
    <div className="space-y-5">
      {/* If No Image Uploaded: Quick Call to Action */}
      {!hasImage && (
        <div className="p-4 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-center space-y-3">
          ...
          <div>
            <h4 className="text-xs font-bold text-white uppercase tracking-wider">Step 1: Upload Image</h4>
            <p className="text-[11px] text-slate-400 mt-1">
              Select or drop an image to start segmenting objects with open-vocabulary text prompts.
            </p>
          </div>
          <label className="cursor-pointer inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold shadow-md transition-all active:scale-95 w-full">
